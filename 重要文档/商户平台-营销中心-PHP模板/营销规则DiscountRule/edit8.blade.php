...
<div id="priceMarketing">
                        ...
                        <div class="item" v-for="(cal,calIndex) in item.calculationInterval" :key="'calculationInterval'+calIndex">
                            ...
                            <div id="box">
                                <div class="input-box" v-for="(time,timeIndex)  in cal.calculation" :key="'calculation'+timeIndex">                         
                                    ...
                                    <span >
                                        <div class="coupon_select select" @click="showSelectGoods(time)" type="0">
                                            <div class="coupon_title font-gray" v-if="!time.giveGoodsList || time.giveGoodsList.length == 0" >选择非油商品</div>
                                            <div class="coupon_title font-gray" v-else >已选择@{{time.giveGoodsList.length}}个商品</div>
                                        </div>
                                    </span>
    ...
    <el-dialog  title="已选择非油商品列表"  append-to-body :visible.sync="selectGoodsDialogVisible" width="900px">
        <div>
            <el-button type="primary" @click="showGoods()">选择商品</el-button>
        </div>
        <div style="height:500px;">
            <el-table height="500"   :data="selectGoodsList"   stripe    style="width: 100%">
                ...
                <el-table-column    prop="BarCode"    label="商品编码"    width="160"></el-table-column>
                <el-table-column    prop="DEFAULTPRICE"    label="当前售价"   >
                    <template slot-scope="scope"  >
                            @{{scope.row.DEFAULTPRICE}}元
                    </template>
                </el-table-column>
                <el-table-column    prop="goodsNum"    label="商品数量"    width="160">
                    <template slot-scope="scope">
                        <el-input-number size="mini" :min="1"   style="width:100px" v-model="scope.row.goodsNum"></el-input-number>
                    </template>
                </el-table-column>
                <el-table-column    prop="isSelected"    label="默认赠送商品"    width="160">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.isSelected" @change="changeSwitch(scope.$index, selectGoodsList)"></el-switch>
                    </template>
                </el-table-column>
    ...
    <el-dialog  title="非油商铺"  append-to-body :visible.sync="goodsDialogVisible" width="1000px">
        <div style="height:590px;">
            <div class="keywordInput">
                ...
                <el-button type="primary" icon="el-icon-search" @click="ajaxGetGoodsList()">搜索</el-button>
            </div>
            <div class="treeBox">
                <el-tree
            ...
            <el-table   v-loading="goodsLoading" :data="goodsList"   stripe    style="height:530px;overflow:auto;width: 100%" @selection-change="goodsSelectionChange">
                    <el-table-column  type="selection"  width="55"></el-table-column>
                    <el-table-column    prop="NAME"    label="商品名称"    width="340"></el-table-column>
        ...
        var $Main = new Vue({
            ...
            methods: {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            if (res.status == 200) {
                                ...
                                that.toAddRule(8);
                                //商品类型
                                that.submit.rule_discount_method = res.data.RuleDiscountMethod
                                //规则名称
                        ...
                        _RuleDiscount.map(function(element,index){
                            ...
                            element.calculationInterval.map(function(cal,index2){
                                cal.calculation.map(function(calcula,index3){
                                   ...
                                   that.submit.rule_discount[index].calculationInterval[index2].calculation[index3].giveGoodsList = calcula.giveGoodsList;
                                   
                                })
                                
                            
                                var PayMethod = _payType;
                                
                                if(PayMethod instanceof Array){
                                    that.submit.rule_discount[index].calculationInterval[index2].PayMethod = PayMethod;
                                }else{

                                    //处理数据中，没有支付方式的要去掉；
                                    var _payList = PayMethod.split(",");
                                    var _newPayList = [];
                                    that.form.payType.forEach(function(item){
                                        if(_payList.includes(item.BH)){
                                            _newPayList.push(item.BH)
                                        }
                                    })
                                    that.submit.rule_discount[index].calculationInterval[index2].PayMethod = _newPayList;
                                ...
                                //处理数据中，没有支付方式的要去掉；
                                var _oilList = cal.oilAndGoods;
                                var _newOilList = [];
                              
                                that.form.oilsList.forEach(function(item){

                                    _oilList.forEach(function(item2){
                                        if(item.oil_id == item2.SKU){
                                            _newOilList.push(item2.SKU)
                                        }
                                
                                    })
                                })
                     
                                that.submit.rule_discount[index].calculationInterval[index2].oilAndGoods = _newOilList;
                    ...
                    this.submit.rule_discount.forEach(function(item){
                        ...
                        item.calculationInterval.forEach(function(cal){
                            ...
                            cal.calculation.forEach(function(calcu){
                               ...
                               if(calcu.giveGoodsList.length == 0){
                                    that.$message.error('请选择赠送商品！');
                                    isNext = false;
                                    throw new Error("ending");//报错，就跳出循环
                                }
                ...
                showSelectGoods(obj){
                    ...
                    obj.giveGoodsList.forEach(function(item,index){
                        _selectGoodsList[index] = {
                            "isSelected":item.ISDefault == "true"?true:false,
                            "BarCode": item.SKU,
                            "DEFAULTPRICE" : item.DEFAULTPRICE,
                            "NAME" : item.GoodsName,
                            "goodsNum" : item.GiveNum,
                        }
                    })
                ...
                btnSureGoods(){
                    ...
                    this.targetObj.giveGoodsList = [];
                    this.selectGoodsList.forEach(function(item,index){
                        that.targetObj.giveGoodsList[index] = {
                            "ISDefault" : item.isSelected,
                            "SKU" : item.BarCode,
                            "DEFAULTPRICE" : item.DEFAULTPRICE,
                            "GoodsName" : item.NAME,
                            "GiveNum" : item.goodsNum,
                        }
                        if(item.isSelected){
                            hasSelected  = true;
                        }
                    ...
                    data.rule_discount.forEach(function(item){
                        item.calculationInterval.forEach(function(item2){
                            ...
                            item2.calculation.forEach(function(calcu){
                               ...
                               //判断返赠
                                calcu.giveGoodsList=$.extend(true,[],calcu.giveGoodsList);
