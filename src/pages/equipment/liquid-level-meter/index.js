...
const LiquidLevelMeterManagement = () => {
  // 状态定义
  ...
  const columns = [
    {
      ...
      width: 120,
      render: (text, record) => (
        <Link to={`/equipment/liquid-level-meter/detail/${record.id}`} className="device-name-link">
          {text}
        </Link>
      )
    },
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Link to={`/equipment/liquid-level-meter/detail/${record.id}`}>
  ...
  return (
    <div className="liquid-level-meter-container">
      <Card>
        {/* 统计卡片 */}
        ...
        {alarmCount > 0 && (
          <Alert
            message={`当前有 ${alarmCount} 台液位仪存在报警，请及时处理`}
            type="warning"
            showIcon
...
export default LiquidLevelMeterManagement; 