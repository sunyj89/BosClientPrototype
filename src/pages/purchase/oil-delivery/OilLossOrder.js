...
const OilLossOrder = () => {
  ...
  const mockData = Array.from({ length: 20 }).map((_, index) => ({
    key: index,
    orderNo: `LS${String(2023001 + index).padStart(6, '0')}`,
    ...
    actualQuantity: Math.floor(4800 + Math.random() * 9800),
    lossQuantity: Math.floor(50 + Math.random() * 200),
    lossRate: (Math.random() * 2).toFixed(2),
    ...
    approvalRemark: index % 3 === 1 ? '属于正常损耗范围' : (index % 3 === 2 ? '损耗率过高，需要调查' : ''),
  }));
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <Row gutter={[16, 16]}>
            <Col span={12}>
              <Form.Item name="lossQuantity" label="损耗数量(L)">
                <Input disabled />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="lossRate" label="损耗率(%)">
                <Input disabled />
              </Form.Item>
            </Col>
          </Row>
      ...
      >
        ...
        >
          ...
          <Form.Item 
            name="reason" 
            label="损耗原因"
            rules={[{ required: true, message: '请选择损耗原因' }]}
          >
            <Select style={{ width: '100%' }} placeholder="请选择损耗原因">
              <Option value="温度变化">温度变化</Option>
              <Option value="运输过程">运输过程</Option>
              <Option value="计量误差">计量误差</Option>
              <Option value="蒸发损耗">蒸发损耗</Option>
            </Select>
          </Form.Item>
          <Form.Item name="remark" label="备注">
